...
router.get('/', async (req, res) => {
    try {
        ...
        const [dbCandidates] = await db_1.pool.query('SELECT * FROM `Candidate` ORDER BY `registeredAt` DESC');
        const [users] = await db_1.pool.query('SELECT `id`, `name` FROM `User`').catch(() => [[]]);
        ...
        const candidates = (dbCandidates || []).map((c) => {
            ...
            let videoUrlVal = profile ? profile.videoUrl : (c.videoUrl || c.Youtube_URL);
            ...
            return {
                ...
                videoUrl: (0, crypto_1.encryptPath)(videoUrlVal),
                ...
                isInvoiceDelivered: invoiceMap.get(c.id)?.some((i) => i.isDelivered) || false,
...
router.post('/promote-from-quick', async (req, res) => {
    try {
        const { quickRegistrationId } = req.body;
        if (!quickRegistrationId) {
            return res.status(400).json({ error: 'quickRegistrationId is required' });
        }
        const [qrRows] = await db_1.pool.query('SELECT * FROM `QuickRegistration` WHERE `id` = ? LIMIT 1', [quickRegistrationId]);
        ...
        const [cands] = await db_1.pool.query('SELECT * FROM `Candidate` WHERE LOWER(`passportNumber`) = LOWER(?) LIMIT 1', [qr.passportNumber]);
        ...
        if (!hasRemoteVideo) {
            try {
                ...
                if (pNum) {
                    const [matchingVideos] = await db_1.pool.query('SELECT videoUrl FROM `PreRegisteredVideo` WHERE UPPER(`passportNumber`) = ? LIMIT 1', [pNum]);
        ...
        if (setClauses.length > 0) {
            ...
            await db_1.pool.query(`UPDATE \`Candidate\` SET ${setClauses.join(', ')} WHERE \`id\` = ?`, params);
        }
        await db_1.pool.query(`UPDATE \`QuickRegistration\` SET \`promotedAt\` = NOW(), \`promotedCandidateId\` = ?, \`verificationStatus\` = 'promoted' WHERE \`id\` = ?`, [candidate.id, quickRegistrationId]);
        res.json({
            success: true,
...
router.post('/', async (req, res) => {
    try {
        ...
        const [passportImageUrl, facePhotoUrl, fullBodyPhotoUrl, cocDocumentUrl, medicalDocumentUrl, candidateIdImageUrl, relativeIdImageUrl, labourIdUrl, videoUrl] = await Promise.all([
            (0, upload_1.uploadToLocal)(body.passportImageUrl, 'passports'),
            (0, upload_1.uploadToLocal)(body.facePhotoUrl, 'faces'),
            (0, upload_1.uploadToLocal)(body.fullBodyPhotoUrl, 'fullbody'),
            (0, upload_1.uploadToLocal)(body.personalInfo.cocDocumentUrl, 'coc'),
            (0, upload_1.uploadToLocal)(body.personalInfo.medicalDocumentUrl, 'medical'),
            (0, upload_1.uploadToLocal)(body.personalInfo.candidateIdImageUrl, 'candidate-id'),
            (0, upload_1.uploadToLocal)(body.personalInfo.relativeIdImageUrl, 'relative-id'),
            (0, upload_1.uploadToLocal)(body.personalInfo.labourIdUrl, 'labour-id'),
        ...
        if (currentCounter === 0) {
            const [lastCand] = await db_1.pool.query('SELECT `shelfId` FROM `Candidate` WHERE `shelfId` IS NOT NULL ORDER BY `shelfId` DESC LIMIT 1');
        ...
        let matchedPreRegisteredVideoUrl = null;
        try {
        ...
        if (userRole === 'calling' || body.personalInfo?.brokerId === 'calling-broker' || body.isCalling) {
            try {
                const [callingBrokers] = await db_1.pool.query("SELECT id FROM Broker WHERE name = 'Calling' LIMIT 1");
                if (callingBrokers && callingBrokers.length > 0) {
                    finalBrokerId = callingBrokers[0].id;
                }
                else {
                    const brokerId = 'cb' + (0, db_1.generateId)().slice(0, 23);
                    await db_1.pool.query('INSERT INTO Broker (id, name, leaderId, createdAt, isLocked) VALUES (?, ?, NULL, NOW(3), 0)', [brokerId, 'Calling']);
        ...
        const candidateId = (0, db_1.generateId)();
        ...
        await db_1.db.insert(db_1.candidate).values(candidateValues);
        if (userRole === 'calling' || body.personalInfo?.brokerId === 'calling-broker' || body.isCalling) {
            ...
            if (validTemplates.includes(templateId.toLowerCase())) {
                try {
                    ...
                    await db_1.db.update(db_1.candidate).set({ cvDeadline: deadline }).where((0, drizzle_orm_1.eq)(db_1.candidate.id, candidateId));
                }
                catch (cvErr) {
        ...
        if (body.quickRegistrationId) {
            try {
                const [qrRows] = await db_1.pool.query(`SELECT \`allowVideo\` FROM \`QuickRegistration\` WHERE \`id\` = ?`, [body.quickRegistrationId]);
                if (qrRows && qrRows.length > 0) {
                    const qrAllowVideo = qrRows[0].allowVideo === 1 || qrRows[0].allowVideo === true;
                    await db_1.db.update(db_1.candidate).set({ allowVideo: qrAllowVideo }).where((0, drizzle_orm_1.eq)(db_1.candidate.id, candidateId));
                }
                await db_1.pool.query(`UPDATE \`QuickRegistration\` SET \`promotedAt\` = NOW(), \`promotedCandidateId\` = ?, \`verificationStatus\` = 'promoted' WHERE \`id\` = ?`, [candidateId, body.quickRegistrationId]);
            }
            catch (promotionError) {
                console.error(`Failed to update QuickRegistration promotion:`, promotionError);
            }
        }
        const [createdCand] = await db_1.pool.query('SELECT * FROM `Candidate` WHERE `id` = ? LIMIT 1', [candidateId]);
...
router.get('/by-passport/:passportNumber', async (req, res) => {
    try {
        ...
        const [rows] = await db_1.pool.query('SELECT givenNames, surname FROM `Candidate` WHERE LOWER(`passportNumber`) = LOWER(?) LIMIT 1', [passportNumber]);
...
router.get('/:id', async (req, res) => {
    try {
        ...
        const isSuperAdmin = role === 'super_admin';
        const { id } = req.params;
        const [rows] = await db_1.pool.query('SELECT * FROM `Candidate` WHERE `id` = ? LIMIT 1', [id]);
        if (!rows || rows.length === 0)
            return res.status(404).json({ error: 'Not found' });
        const c = rows[0];
        let broker = null;
        if (c.brokerId) {
            const [brokerRows] = await db_1.pool.query('SELECT `id`, `name`, `isLocked` FROM `Broker` WHERE `id` = ? LIMIT 1', [c.brokerId]);
        ...
        const [generatedCVs] = await db_1.pool.query('SELECT `id`, `templateId` FROM `GeneratedCV` WHERE `candidateId` = ? ORDER BY `createdAt` DESC LIMIT 1', [id]);
        let registeredByName = 'Admin';
        if (c.registeredById) {
            try {
                const [userRows] = await db_1.pool.query('SELECT `name` FROM `User` WHERE `id` = ? LIMIT 1', [c.registeredById]);
        ...
        let uploadedFacePhotoUrl = null;
        let uploadedFullBodyPhotoUrl = null;
        let uploadedVideoUrl = null;
        if (c.passportNumber) {
            try {
                const pNum = c.passportNumber.trim().toUpperCase();
                const [profileRows] = await db_1.pool.query(`SELECT facePhotoUrl, fullBodyPhotoUrl, videoUrl FROM \`UploadedVideoProfile\` WHERE UPPER(\`passportNumber\`) = ? LIMIT 1`, [pNum]);
        ...
        const candidate = {
            ...
            passportImageUrl: (0, crypto_1.encryptPath)(c.passportImageUrl),
            facePhotoUrl: (0, crypto_1.encryptPath)(uploadedFacePhotoUrl || c.facePhotoUrl),
            fullBodyPhotoUrl: (0, crypto_1.encryptPath)(uploadedFullBodyPhotoUrl || c.fullBodyPhotoUrl),
            cocDocumentUrl: (0, crypto_1.encryptPath)(c.cocDocumentUrl),
            medicalDocumentUrl: (0, crypto_1.encryptPath)(c.medicalDocumentUrl),
            candidateIdImageUrl: (0, crypto_1.encryptPath)(c.candidateIdImageUrl),
            relativeIdImageUrl: (0, crypto_1.encryptPath)(c.relativeIdImageUrl),
            ...
            isRequested: c.isRequested === 1 || c.isRequested === true,
            visaOrContractNumber: c.visaOrContractNumber || null,
            videoUrl: (0, crypto_1.encryptPath)(uploadedVideoUrl || youtubeUrl || c.videoUrl || null),
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const [existingRows] = await db_1.pool.query('SELECT visaDate, registeredById FROM Candidate WHERE id = ? LIMIT 1', [id]);
        ...
        if (facePhotoUrl)
            updateFields.facePhotoUrl = facePhotoUrl;
        if (fullBodyPhotoUrl)
            updateFields.fullBodyPhotoUrl = fullBodyPhotoUrl;
        if (cocDocumentUrl)
            updateFields.cocDocumentUrl = cocDocumentUrl;
        if (medicalDocumentUrl)
            updateFields.medicalDocumentUrl = medicalDocumentUrl;
        ...
        if (priceVal !== undefined)
            updateFields.price = priceVal;
        if (laborIdVal !== undefined)
            updateFields.laborID = laborIdVal || null;
        if (!existingCandidate?.registeredById && registeredById)
            updateFields.registeredById = registeredById;
        ...
        await db_1.db.update(db_1.candidate).set(updateFields).where((0, drizzle_orm_1.eq)(db_1.candidate.id, id));
        if (facePhotoUrl || fullBodyPhotoUrl) {
            try {
                const cvUpdate = {};
                if (facePhotoUrl)
                    cvUpdate.facePhotoUrl = facePhotoUrl;
                if (fullBodyPhotoUrl)
                    cvUpdate.fullBodyPhotoUrl = fullBodyPhotoUrl;
                await db_1.db.update(db_1.generatedCv).set(cvUpdate).where((0, drizzle_orm_1.eq)(db_1.generatedCv.candidateId, id));
            }
            catch (_) { }
        }
        const [updatedRows] = await db_1.pool.query('SELECT * FROM Candidate WHERE id = ? LIMIT 1', [id]);
...
router.patch('/bulk-cv-downloaded', async (req, res) => {
    try {
        ...
        if (cvDownloaded) {
            for (const id of candidateIds) {
                try {
                    const existingCVs = await db_1.db.select().from(db_1.generatedCv).where((0, drizzle_orm_1.eq)(db_1.generatedCv.candidateId, id));
                    if (existingCVs.length === 0) {
                        const [cands] = await db_1.pool.query('SELECT facePhotoUrl, fullBodyPhotoUrl FROM Candidate WHERE id = ? LIMIT 1', [id]);
                        if (cands && cands.length > 0) {
                            await db_1.db.insert(db_1.generatedCv).values({
                                candidateId: id,
                                templateId: 'alm',
...
router.patch('/:id', async (req, res) => {
    try {
        ...
        if (body.medicalStatus === 'Unfit') {
            ...
            await db_1.db.delete(db_1.generatedCv).where((0, drizzle_orm_1.eq)(db_1.generatedCv.candidateId, id));
        }
        ...
        if (isLockedVal !== undefined)
            updateFields.isLocked = isLockedVal;
        if (cvDownloadedVal !== undefined)
            updateFields.cvDownloaded = cvDownloadedVal;
        if (priceVal !== undefined)
            updateFields.price = priceVal;
        if (laborIdVal !== undefined)
            updateFields.laborID = laborIdVal || null;
        if (allowVideoVal !== undefined)
            updateFields.allowVideo = Boolean(allowVideoVal);
        if (deployedDateVal !== undefined)
            updateFields.deployedDate = deployedDateVal ? new Date(deployedDateVal) : null;
        if (videoUrlVal !== undefined) {
            const sanitizedVideoUrl = (0, crypto_1.sanitizeIncomingPath)(videoUrlVal);
        ...
        if (quickVideoUrlVal !== undefined) {
            updateFields.quickVideoUrl = (0, crypto_1.sanitizeIncomingPath)(quickVideoUrlVal) || null;
        }
...
router.delete('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        await db_1.db.delete(db_1.generatedCv).where((0, drizzle_orm_1.eq)(db_1.generatedCv.candidateId, id)).catch(() => { });
        await db_1.db.delete(db_1.invoice).where((0, drizzle_orm_1.eq)(db_1.invoice.candidateId, id)).catch(() => { });
        await db_1.db.delete(db_1.notification).where((0, drizzle_orm_1.eq)(db_1.notification.candidateId, id)).catch(() => { });
        await db_1.pool.query(`UPDATE \`QuickRegistration\` SET \`promotedCandidateId\` = NULL, \`verificationStatus\` = 'pending' WHERE \`promotedCandidateId\` = ?`, [id]).catch(() => { });
        await db_1.db.delete(db_1.candidate).where((0, drizzle_orm_1.eq)(db_1.candidate.id, id));
