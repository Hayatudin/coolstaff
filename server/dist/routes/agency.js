...
async function ensureCandidateColumns() {
    try {
        const [columns] = await db_1.pool.query('SHOW COLUMNS FROM `Candidate`');
        ...
        for (const col of requiredColumns) {
            if (!existingFields.has(col.name.toLowerCase())) {
                ...
                await db_1.pool.query(`ALTER TABLE \`Candidate\` ADD COLUMN \`${col.name}\` ${col.definition}`);
            }
        }
    }
    catch (err) {
...
function inferAgencyFromEmail(email) {
    const e = email.toLowerCase();
    if (e.includes('ussus'))
        return 'ussus';
    if (e.includes('khuzam') || e.includes('ku2'))
        return 'ku2';
    if (e.includes('kafaat') || e.includes('ka7'))
        return 'ka7';
    if (e.includes('almersah') || e.includes('alaalam') || e.includes('alm'))
        return 'alm';
    if (e.includes('almala'))
        return 'almala';
    if (e.includes('rayaat') || e.includes('ra'))
        return 'ra';
    if (e.includes('shablan'))
        return 'al-shablan';
    if (e.includes('vision'))
        return 'vision';
    if (e.includes('ma'))
        return 'ma';
    return null;
...
async function resolveAndHealAgency(user) {
    if (user.agency)
        return user.agency;
    if (user.email) {
        ...
        if (inferred) {
            ...
            try {
                await db_1.db.update(db_1.user).set({ agency: inferred }).where((0, drizzle_orm_1.eq)(db_1.user.id, user.id));
            }
            catch (err) {
...
router.get('/debug-info', async (req, res) => {
    try {
        ...
        const candCountRes = await db_1.db.select({ value: (0, drizzle_orm_1.count)() }).from(db_1.candidate);
        const cvCountRes = await db_1.db.select({ value: (0, drizzle_orm_1.count)() }).from(db_1.generatedCv);
        const uniqueTemplates = await db_1.db
            .select({
            templateId: db_1.generatedCv.templateId,
            count: (0, drizzle_orm_1.count)(db_1.generatedCv.id),
        })
            .from(db_1.generatedCv)
            .groupBy(db_1.generatedCv.templateId);
        const sampleCandidates = await db_1.db
            .select({
            id: db_1.candidate.id,
            givenNames: db_1.candidate.givenNames,
            surname: db_1.candidate.surname,
            agency: db_1.candidate.agency,
        })
            .from(db_1.candidate)
            .limit(5);
        res.json({
...
router.get('/candidates', async (req, res) => {
    try {
        ...
        const sqlParams = [];
        const whereClauses = ['c.`agencySelected` = 1'];
        if (role === 'agency') {
        ...
        const [rawCands] = await db_1.pool.query(sqlQuery, sqlParams);
        let dbCandidates = [];
        if (rawCands.length > 0) {
            const candidateIds = rawCands.map((c) => c.id);
            const allCVs = await db_1.db
                .select({ id: db_1.generatedCv.id, templateId: db_1.generatedCv.templateId, candidateId: db_1.generatedCv.candidateId })
                .from(db_1.generatedCv)
                .where((0, drizzle_orm_1.inArray)(db_1.generatedCv.candidateId, candidateIds));
            const allInvoices = await db_1.db
                .select({ candidateId: db_1.invoice.candidateId, lmisQrCodeUrl: db_1.invoice.lmisQrCodeUrl })
                .from(db_1.invoice)
                .where((0, drizzle_orm_1.inArray)(db_1.invoice.candidateId, candidateIds));
        ...
        res.json(dbCandidates.map((c) => {
            const invoicesList = c.invoices || [];
            const hasQrCode = invoicesList.some((inv) => inv.lmisQrCodeUrl && inv.lmisQrCodeUrl.trim() !== '');
            return {
                id: c.id,
                ...
                videoUrl: (0, crypto_1.encryptPath)(c.videoUrl || c.Youtube_URL || null) || null,
...
router.get('/available-candidates', async (req, res) => {
    try {
        ...
        res.json(dbCandidates.map((c) => {
            ...
            const videoUrlVal = profile ? profile.videoUrl : (c.videoUrl || c.Youtube_URL || null);
            ...
            return {
                ...
                city: c.city,
                dateOfBirth: c.dateOfBirth ? new Date(c.dateOfBirth).toISOString() : null,
                videoUrl: (0, crypto_1.encryptPath)(videoUrlVal) || null,
                registeredAt: c.registeredAt ? new Date(c.registeredAt).toISOString() : null,
                facePhotoUrl: (0, crypto_1.encryptPath)(facePhotoUrlVal) || null,
                fullBodyPhotoUrl: (0, crypto_1.encryptPath)(fullBodyPhotoUrlVal) || null,
                passportImageUrl: (0, crypto_1.encryptPath)(c.passportImageUrl) || null,
...
router.post('/candidates/:id/select', async (req, res) => {
    try {
        ...
        const [candRow] = await db_1.pool.query('SELECT * FROM `Candidate` WHERE `id` = ? LIMIT 1', [id]);
        if (!candRow) {
        ...
        if (role === 'agency') {
            ...
            if (candidate.brokerId) {
                const [brokerRow] = await db_1.pool.query('SELECT `isLocked` FROM `Broker` WHERE `id` = ? LIMIT 1', [candidate.brokerId]);
        ...
        await db_1.db.update(db_1.candidate).set({ agencySelected: true }).where((0, drizzle_orm_1.eq)(db_1.candidate.id, id));
        ...
        const [updated] = await db_1.pool.query('SELECT * FROM `Candidate` WHERE `id` = ? LIMIT 1', [id]);
        res.json(updated);
    }
    catch (err) {
...
router.post('/candidates/:id/deselect', async (req, res) => {
    try {
        const session = await (0, auth_helper_1.getSession)(req);
        if (!session || !session.user) {
            return res.status(401).json({ error: 'Unauthorized' });
        ...
        const candidate = candRow;
        const agencyLabel = agencyName ? agencyName.toUpperCase() : 'AGENCY';
        await db_1.db.update(db_1.candidate).set({ agencySelected: false }).where((0, drizzle_orm_1.eq)(db_1.candidate.id, id));
        await db_1.db.insert(db_1.notification).values({
...
router.patch('/candidates/:id', async (req, res) => {
    try {
        ...
        const { id } = req.params;
        const { embassyIssue, cocStatus, medicalStatus, tasheerStatus, wakalaStatus, qrCodeStatus, selectedType, travelDate, agencyStatus } = req.body;
        const updateData = {};
        if (embassyIssue !== undefined)
            updateData.embassyIssue = embassyIssue;
        if (cocStatus !== undefined)
            updateData.cocStatus = cocStatus;
        if (medicalStatus !== undefined)
            updateData.medicalStatus = medicalStatus;
        if (tasheerStatus !== undefined)
            updateData.tasheerStatus = tasheerStatus;
        if (wakalaStatus !== undefined)
            updateData.wakalaStatus = wakalaStatus;
        if (qrCodeStatus !== undefined)
            updateData.qrCodeStatus = qrCodeStatus;
        if (selectedType !== undefined)
            updateData.selectedType = selectedType;
        if (travelDate !== undefined) {
            updateData.travelDate = travelDate ? new Date(travelDate) : null;
        }
        if (agencyStatus !== undefined)
            updateData.agencyStatus = agencyStatus;
        if (Object.keys(updateData).length > 0) {
            await db_1.db.update(db_1.candidate).set(updateData).where((0, drizzle_orm_1.eq)(db_1.candidate.id, id));
        }
        const [rawCands] = await db_1.pool.query('SELECT * FROM `Candidate` WHERE `id` = ? LIMIT 1', [id]);
        if (rawCands[0]) {
            const c = rawCands[0];
            res.json({
                id: c.id,
