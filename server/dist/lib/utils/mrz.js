...
exports.COUNTRY_MAP = void 0;
exports.cleanNumericString = cleanNumericString;
exports.formatDate = formatDate;
exports.preprocessOcrLine = preprocessOcrLine;
exports.normalizeLine = normalizeLine;
exports.mrzScore = mrzScore;
exports.findMrzLines = findMrzLines;
exports.preCleanMrzLine1 = preCleanMrzLine1;
exports.cleanPassportNumber = cleanPassportNumber;
exports.cleanName = cleanName;
exports.trySplitMisreadChevron = trySplitMisreadChevron;
exports.recoverGivenNames = recoverGivenNames;
exports.findCountryCodeAnchor = findCountryCodeAnchor;
exports.parseGender = parseGender;
exports.parseGenderFromText = parseGenderFromText;
exports.extractDatesAndGender = extractDatesAndGender;
exports.COUNTRY_MAP = {
...
function mrzScore(raw) {
    if (/[^\x00-\x7F]/.test(raw))
        return 0;
    if (/[a-z]/.test(raw) && (raw.match(/[a-z]/g) || []).length > 5)
        return 0;
...
function preCleanMrzLine1(line) {
...
function trySplitMisreadChevron(name) {
...
function recoverGivenNames(rawGivenNames) {
...
function findCountryCodeAnchor(line2) {
    const range = line2.substring(7, 16);
    for (const code of Object.keys(exports.COUNTRY_MAP)) {
...
function parseGender(char) {
    if (!char)
        return '';
    const c = char.toUpperCase();
    if (c === 'M' || c === 'N' || c === 'H' || c === '1')
        return 'Male';
    if (c === 'F' || c === 'E' || c === 'P' || c === 'R' || c === 'K' || c === '7')
        return 'Female';
    return '';
}
function parseGenderFromText(text) {
    ...
    if (cleaned.includes('F') || cleaned.includes('E') || cleaned.includes('P') || cleaned.includes('R') || cleaned.includes('K'))
        return 'Female';
    if (cleaned.includes('M') || cleaned.includes('N') || cleaned.includes('H'))
        return 'Male';
...
function extractDatesAndGender(rest) {
    const candidates = [];
