...
async function ensureDatabaseSchema() {
    ...
    const executeRaw = async (query, params = []) => {
        return await db_1.pool.query(query, params);
    };
    ...
    try {
        await executeRaw(`ALTER TABLE \`User\` ADD COLUMN \`agency\` VARCHAR(191) NULL`);
    ...
    try {
        const { hashPassword } = await Promise.resolve().then(() => __importStar(require('better-auth/crypto')));
        const hashedPass = await hashPassword('muju1212');
        const users = await queryRaw(`SELECT id FROM \`User\` WHERE email = 'hayuuj0@gmail.com'`);
        let targetUserId;
        if (users.length === 0) {
            targetUserId = 'c' + Math.random().toString(36).substring(2, 14);
            await executeRaw(`INSERT INTO \`User\` (\`id\`, \`name\`, \`email\`, \`emailVerified\`, \`role\`, \`createdAt\`, \`updatedAt\`) VALUES (?, 'Hayatu', 'hayuuj0@gmail.com', 1, 'super_admin', NOW(), NOW())`, [targetUserId]);
    ...
    try {
        const columns = await queryRaw(`SHOW COLUMNS FROM \`Candidate\` LIKE 'flaggedAt'`);
        if (columns.length === 0) {
            await executeRaw(`ALTER TABLE \`Candidate\` ADD COLUMN \`flaggedAt\` DATETIME(3) NULL`);
    ...
    try {
        ...
        try {
            const columns = await queryRaw(`SHOW COLUMNS FROM \`PreRegisteredVideo\``);
        ...
        if (hasFullName) {
            try {
                await executeRaw(`ALTER TABLE \`PreRegisteredVideo\` DROP INDEX \`PreRegisteredVideo_fullName_key\``);
            }
            catch (_) { }
            try {
                await executeRaw(`ALTER TABLE \`PreRegisteredVideo\` CHANGE COLUMN \`fullName\` \`passportNumber\` VARCHAR(191) NOT NULL`);
                await executeRaw(`ALTER TABLE \`PreRegisteredVideo\` ADD UNIQUE KEY \`PreRegisteredVideo_passportNumber_key\` (\`passportNumber\`)`);
      ...
      `);
        ...
        if (hasPassportNumber) {
            if (!hasFacePhoto) {
                try {
                    await executeRaw(`ALTER TABLE \`PreRegisteredVideo\` ADD COLUMN \`facePhotoUrl\` TEXT NULL`);
            ...
            if (!hasFullBodyPhoto) {
                try {
                    await executeRaw(`ALTER TABLE \`PreRegisteredVideo\` ADD COLUMN \`fullBodyPhotoUrl\` TEXT NULL`);
    ...
    // 9. Create Passport Table
    try {
        try {
            const cols = await queryRaw('SHOW COLUMNS FROM `Passport`');
            const hasShelfNo = cols.some(c => c.Field === 'shelfNo');
            if (!hasShelfNo) {
                console.log('🔄 Old Passport table detected. Recreating to match new schema...');
                await executeRaw('DROP TABLE IF EXISTS `Passport`');
            }
        }
        catch (_) { }
        await executeRaw(`
    ...
    for (const col of candidateColumns) {
        try {
            await executeRaw(`ALTER TABLE \`Candidate\` ADD COLUMN \`${col.name}\` ${col.type}`);
    ...
    for (const col of quickRegColumns) {
        try {
            await executeRaw(`ALTER TABLE \`QuickRegistration\` ADD COLUMN \`${col.name}\` ${col.type}`);
    ...
    // Auto-migration for candidate video paths using Drizzle
    try {
        console.log('🔄 Running auto-migration for existing candidates with missing videos...');
        const candidates = await queryRaw(`SELECT id, passportNumber, quickVideoUrl, Youtube_URL FROM \`Candidate\` WHERE quickVideoUrl IS NULL OR quickVideoUrl = '' OR Youtube_URL IS NULL OR Youtube_URL = ''`);
        console.log(`🔍 Found ${candidates.length} candidates with missing video fields to check.`);
        let migrationCount = 0;
        for (const cand of candidates) {
            if (!cand.passportNumber)
                continue;
            const quickRegs = await db_1.db
                .select({ videoUrl: db_1.quickRegistration.videoUrl })
                .from(db_1.quickRegistration)
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(db_1.quickRegistration.passportNumber, cand.passportNumber), (0, drizzle_orm_1.isNotNull)(db_1.quickRegistration.videoUrl)))
                .limit(1);
            const quickReg = quickRegs[0];
            if (quickReg && quickReg.videoUrl) {
                const isLocalVideo = quickReg.videoUrl.startsWith('/uploads');
                if (isLocalVideo) {
                    await executeRaw(`UPDATE \`Candidate\` SET \`quickVideoUrl\` = ? WHERE \`id\` = ?`, [quickReg.videoUrl, cand.id]);
                }
                else {
                    await executeRaw(`UPDATE \`Candidate\` SET \`Youtube_URL\` = ? WHERE \`id\` = ?`, [quickReg.videoUrl, cand.id]);
                }
                migrationCount++;
            }
        }
    ...
    `);
        const backfilledCount = result?.affectedRows || 0;
