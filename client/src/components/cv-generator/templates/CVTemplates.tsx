    ...
    <div className="bg-white w-full aspect-[210/297] p-8 text-[10px] leading-relaxed" style={{fontFamily:'Georgia, serif'}}>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div><h3 className="font-bold text-gray-800 border-b border-gray-300 pb-1 mb-2">PERSONAL INFORMATION</h3>
          ...
          <p><strong>Religion:</strong> {pi.religion}</p><p><strong>Passport No:</strong> <span className="font-black font-roboto text-[16px] tracking-wider" style={{ fontFamily: "'Roboto', sans-serif", fontWeight: '900', fontSize: '16px' }}>{p.passportNumber}</span></p>
        </div>
  ...
  return (
    <div className="bg-white w-full aspect-[210/297] text-[10px] leading-relaxed overflow-hidden" style={{fontFamily:'Inter, sans-serif'}}>
      ...
      <div className="p-6 space-y-4">
        ...
        <div><h3 className="text-xs font-bold text-indigo-600 uppercase mb-1">Passport</h3><p className="text-gray-700">No: <span className="font-black font-roboto text-[16px] tracking-wider" style={{ fontFamily: "'Roboto', sans-serif", fontWeight: '900', fontSize: '16px' }}>{p.passportNumber}</span> | Issued: {formatDate(p.dateOfIssue)} | Expires: {formatDate(p.dateOfExpiry)}</p></div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="bg-white w-full aspect-[210/297] flex text-[10px] leading-relaxed overflow-hidden" style={{fontFamily:'Inter, sans-serif'}}>
      ...
      <div className="flex-1 p-6 space-y-4">
        ...
        <div><h3 className="text-xs font-bold text-gray-800 mb-2">PASSPORT</h3><p>No: <span className="font-black font-roboto text-[16px] tracking-wider" style={{ fontFamily: "'Roboto', sans-serif", fontWeight: '900', fontSize: '16px' }}>{p.passportNumber}</span> | Valid: {formatDate(p.dateOfIssue)} - {formatDate(p.dateOfExpiry)}</p></div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="bg-white w-full aspect-[210/297] p-10 text-[10px] leading-loose" style={{fontFamily:'Inter, sans-serif'}}>
      ...
      <div className="space-y-5">
        {[{t:'Personal',c:`${formatDate(p.dateOfBirth)} • ${p.gender} • ${resolvedNationality} • ${pi.maritalStatus} • ${pi.religion}`},{t:'Education',c:pi.educationLevel},{t:'Languages',c:pi.languages.join(', ')},{t:'Experience',c:expStr},{t:'Skills',c:pi.skills.join(', ')},{t:'Passport',c:''}].map(s=>(
          <div key={s.t}>
            <h3 className="text-[9px] font-semibold uppercase tracking-widest text-gray-400 mb-1">{s.t}</h3>
            <p className="text-gray-700">
              {s.t === 'Passport' ? (
                <><span className="font-black font-roboto text-[16px] tracking-wider" style={{ fontFamily: "'Roboto', sans-serif", fontWeight: '900', fontSize: '16px' }}>{p.passportNumber}</span> — Valid until {formatDate(p.dateOfExpiry)}</>
              ) : s.c}
            </p>
          </div>
        ))}
      ...
      <div className="p-7">
        ...
        <div className="grid grid-cols-2 gap-5">
          ...
          <div className="space-y-3">
            ...
            <div><h3 className="text-xs font-bold text-amber-700 border-b border-amber-200 pb-1 mb-2">Passport Details</h3><p>No: <span className="font-black font-roboto text-[16px] tracking-wider" style={{ fontFamily: "'Roboto', sans-serif", fontWeight: '900', fontSize: '16px' }}>{p.passportNumber}</span></p><p>Issued: {formatDate(p.dateOfIssue)}</p><p>Expires: {formatDate(p.dateOfExpiry)}</p></div>
          </div>
        </div>
      </div>
